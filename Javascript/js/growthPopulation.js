   ...
   // isHeader=false;
	   else
	    {
	 	  	if(!(lineRecords[header.indexOf('Country Name')].split("\"")[1]=='European Union'))
	 	  	{
					...
					if ([header[i]]=='Population (Millions) - 2010')
					{
					 	tempData[header[i]]=lineRecords[i].split("\"")[1];
					}
					if ([header[i]]=='Population (Millions) - 2011')
					{
					 	tempData[header[i]]=lineRecords[i].split("\"")[1];
					}
					if ([header[i]]=='Population (Millions) - 2012')
					{
					 	tempData[header[i]]=lineRecords[i].split("\"")[1];
					}
	...
	if(flag)
	{
		jsonData.push(tempData);
	 	fs.writeFileSync("json/growthPopulation.json",JSON.stringify(jsonData),encoding="utf8");
 	}
 	tempData={};
